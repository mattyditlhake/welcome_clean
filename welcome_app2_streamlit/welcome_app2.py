...
"""
Welcome Specialist name cleanup change
--------------------------------------
Some source files mark a Welcome Specialist with an asterisk, either before
or after the name, for example "*Jason Ngumbela" or "Nicole Tsele*". The
asterisk is not part of the actual specialist name, and leaving it in the
output creates duplicate-looking specialists in reports and dashboards.

This cleanup removes all "*" characters from the Welcome Specialist value and
normalizes extra spaces. It is applied during extraction and again before the
final outputs are returned, so both initial-form rows and auth rows use the
same clean specialist names.
"""
def clean_welcome_specialist_name(v) -> str:
    return norm(v).replace("*", "").strip()


def clean_welcome_specialist_column(df: pd.DataFrame) -> pd.DataFrame:
    if "Welcome Specialist" not in df.columns:
        return df

    out = df.copy()
    out["Welcome Specialist"] = out["Welcome Specialist"].map(clean_welcome_specialist_name)
    return out
    ...
    while r < len(grid):
        ...
        while rr < len(grid):
            ...
            if non_empty <= 1:
                ...
                blank_streak = 0
                name = clean_welcome_specialist_name(row[date_label_col]) if date_label_col < len(row) else ""
    ...
    while r < len(grid):
        ...
        while rr < len(grid):
            ...
            if non_empty <= 1:
                ...
                blank_streak = 0
                name = clean_welcome_specialist_name(row[name_col]) if name_col < len(row) else ""
                if name and not name.lower().startswith("total"):
